...
| `buckets-srv` | bin | axum 0.8, tokio, sea-orm 1.x, tower-http, dotenvy, dashmap, jsonwebtoken | 统一 HTTP API 服务：Gateway（文件上传网关）+ Web（管理后台），统一认证中间件 |
...
precheck(file_md5, file_size, chunk_size, user_id)
  ...
  │
  ├── SELECT * FROM objects WHERE md5 = ? AND bucket = 'default' AND status = 'active'
  │     │
  │     ├── 存在 → INSERT IGNORE INTO user_objects (user_id, object_id)
  │     │          返回 { exists: true, object_id, object_url }
  │     │          客户端直接完成，无需上传（秒传）
  │     │
  │     └── 不存在 → 继续检查 upload_tasks
  │
  └── SELECT * FROM upload_tasks WHERE file_md5 = ? AND user_id = ?
        │
...
- MySQL 中以 JSON 格式 `MEDIUMTEXT` 存储
...
merge(task_id, file_name, file_md5, file_size, content_type)
  ...
  └── spawn_blocking 后台执行:
        ...
        │             │     → INSERT INTO objects + INSERT INTO user_objects
        │     │         → status = 'completed'
...
STS 签发时:
  ...
  密钥 = `users.secret_key`（64 字符 hex，SQL 预设）
...
**gc_clean**（30 分钟间隔）：
```
SELECT * FROM upload_tasks WHERE expires_at < NOW() AND status NOT IN ('completed', 'expired') LIMIT 100
  │
