...
本文档详细描述 buckets **分片上传**（chunked upload）的完整链路，从 CLI 客户端到服务端各阶段的设计与实现。

Tus 可恢复上传见 [upload-tus.md](upload-tus.md)。
...
| file_md5 | VARCHAR(64) | 文件 MD5（分片上传由客户端预计算提供） |
| file_size | BIGINT | 文件大小（字节） |
| chunk_size | BIGINT | 分片大小（字节） |
| chunk_count | BIGINT | 总分片数 |
...
| upload_method | VARCHAR(32) | `chunked`（分片上传） |
| current_offset | BIGINT | 当前已上传数据量（分片上传不使用，始终为 0） |
...
pub struct UploadTask {
    ...
    pub upload_method: String,     // "chunked"
...
| upload_method | VARCHAR(32) | `chunked`（分片上传） |
| image_width/height | BIGINT | 图片尺寸（像素） |
| image_type | VARCHAR(32) | 图片类型 |
...
### 上传速率限制 (`ratelimit::upload_ratelimit`)
...
```json
{
    ...
    "data": {
        ...
        "object_key": "data/objects/1/20260614/660e8400-e29b-41d4-a716-446655440001.mp4",
...
5. 创建 upload_tasks 记录 (status = "initialized", upload_method = "chunked")
...
```json
{
    ...
    "data": {
        "exists": true,
        "object_id": "660e8400-...",
        "storage_path": "data/objects/1/20260614/660e8400-....mp4",
        "task_id": null,
        "uploaded_chunks": [],
        "chunk_size": 8388608
    }
...
12. ORM 事务写入:
    ORM 事务内依次执行:
       object::ActiveModel insert (upload_method = 'chunked')
...
| `buckets-srv/src/api/sts.rs` | STS 端点 |
